...
def my_main(sc,
    ...
    """
    print(vehicle_id)
    printrdd = filteredRDD 
    [print(x) for x in printrdd.collect()]
    """

    # Discard fields that we do not need to make it faster
    filteredRDD = filteredRDD.map(lambda x: (x[0], x[1], x[6], x[8], x[2]))
    ...
    pairedRDD = sortedRDD.map(lambda x: ((x[1], x[3], x[4]), x))


    #prdd =pairedRDD 
    #[print(x) for x in prdd.collect()]
    ...
    reducedRDD = pairedRDD.reduceByKey(lambda x, y: x if '' != x[4] else y)\
