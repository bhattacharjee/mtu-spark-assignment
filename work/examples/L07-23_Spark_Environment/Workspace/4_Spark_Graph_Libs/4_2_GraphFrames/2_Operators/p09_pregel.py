...
# FUNCTION my_main
# ------------------------------------------
def my_main(spark, my_dataset_dir, num_iterations):
    ...
    my_verticesDF = my_edgesDF.select(my_edgesDF["src"]).withColumnRenamed("src", "id").dropDuplicates().withColumn(
        "value", pyspark.sql.functions.lit(1))
    ...
    my_resultDF = myGF.pregel \
        .setMaxIter(num_iterations) \
        .withVertexColumn("result", pyspark.sql.functions.col("id"), pyspark.sql.functions.col("result") + Pregel.msg()) \
        .sendMsgToSrc(graphframes.lib.Pregel.dst("result")) \
        .sendMsgToDst(graphframes.lib.Pregel.src("result")) \
        .aggMsgs(pyspark.sql.functions.sum(graphframes.lib.Pregel.msg())) \
        .run()
...
if __name__ == '__main__':
    ...
    my_main(spark, my_dataset_dir, num_iterations)
