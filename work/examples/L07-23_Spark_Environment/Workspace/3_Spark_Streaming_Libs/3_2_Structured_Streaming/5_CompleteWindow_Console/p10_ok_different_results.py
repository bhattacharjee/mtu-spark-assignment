...
# FUNCTION my_model
# ------------------------------------------
def my_model(spark, monitoring_dir, time_step_interval, window_duration, sliding_duration, my_timestamp):
    ...
    aggSDF = wordsSDF.groupBy(pyspark.sql.functions.window("my_time", my_window_duration_frequency, my_sliding_duration_frequency),
                              pyspark.sql.functions.col("word")
                             )\
                     .count()

    # 8. Operation T5: We add the new columns for making the window more clear
    formattedSDF = aggSDF.withColumn("window_start", pyspark.sql.functions.col("window").start.cast("string")) \
...
def my_main(spark,
    ...
    dsw = my_model(spark, monitoring_dir, time_step_interval, window_duration, sliding_duration, my_timestamp)
...
if __name__ == '__main__':
    # 1.1. We specify the time interval each new generated batch is processed.
    time_step_interval = 5

    # 1.2. window_duration, i.e., how many previous batches of data are considered on each window.
    window_duration = 2

    # 1.3. sliding duration, i.e., how frequently the new DStream computes results.
    sliding_duration = 1
