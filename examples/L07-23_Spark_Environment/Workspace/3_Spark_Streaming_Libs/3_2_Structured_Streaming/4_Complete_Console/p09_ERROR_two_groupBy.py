...
def my_model(spark, monitoring_dir, time_step_interval):
    ...
    # 7. Operation T4: Extra Column
    randomSDF = aggSDF.withColumn("newNumber", pyspark.sql.functions.when(pyspark.sql.functions.rand() > 0.5, 1).otherwise(0))

    # 8. Operation T5: ERROR - Extra Chained Aggregation
    agg2SDF = randomSDF.groupBy(pyspark.sql.functions.col("newNumber")) \
                       .count()

    # 9. Operation T6: We sort them by the starting time of the window
    solutionSDF = agg2SDF.orderBy(pyspark.sql.functions.col("count").desc())
