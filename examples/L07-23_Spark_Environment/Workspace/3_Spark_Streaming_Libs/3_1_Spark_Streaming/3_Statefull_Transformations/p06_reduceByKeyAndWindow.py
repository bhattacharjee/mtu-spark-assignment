...
def my_model(ssc, monitoring_dir, result_dir, time_step_interval, window_duration, sliding_duration):
    ...
    # 4. Operation T3: reduceByKeyAndWindow
    solutionDStream = pairWordsDStream.reduceByKeyAndWindow(lambda x, y: x + y,
                                                            lambda x, y: x - y,
                                                            window_duration * time_step_interval,
                                                            sliding_duration * time_step_interval
                                                            )

    # 5. Operation P1: persist
    solutionDStream.persist(pyspark.StorageLevel.MEMORY_AND_DISK)

    # 6. Operation A1: pprint
    solutionDStream.pprint()

    # 7. Operation A2: saveAsTextFiles
