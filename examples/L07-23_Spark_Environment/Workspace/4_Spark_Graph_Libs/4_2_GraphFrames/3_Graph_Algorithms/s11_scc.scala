...
object MyProgram {
  ...
  // FUNCTION myMain
  // ------------------------------------------
  def myMain(spark: SparkSession, myDatasetDir: String, numIterations: Int): Unit = {
    ...
    val solutionVerticesDF = myGF.stronglyConnectedComponents.maxIter(numIterations).run()
