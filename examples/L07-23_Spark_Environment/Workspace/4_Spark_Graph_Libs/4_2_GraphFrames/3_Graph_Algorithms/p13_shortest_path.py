...
def my_main(spark, my_dataset_dir):
    ...
    my_verticesDF = my_edgesDF.select(my_edgesDF["src"]) \
      .withColumn("id", pyspark.sql.functions.col("src").cast(pyspark.sql.types.StringType())).drop("src") \
      .dropDuplicates() \
    ...
    # 2. Operation T4: We run the We run the shortest path algorithm
    solution_verticesDF = myGF.shortestPaths(["1"])

    # 3. Operation A1: We display the solution
    solution_verticesDF.show()
