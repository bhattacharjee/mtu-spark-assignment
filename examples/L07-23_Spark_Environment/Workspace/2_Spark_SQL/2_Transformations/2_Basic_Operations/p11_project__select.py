...
def project_subset_columns(spark):
    ...
    resultDF = inputDF.select(pyspark.sql.functions.col("LikesTennis"),
                              pyspark.sql.functions.col("Name")
                             )
...
def project_subset_columns_rename_one(spark):
    ...
    resultDF = inputDF.select(pyspark.sql.functions.col("LikesTennis"),
                              pyspark.sql.functions.col("Name").alias("NewName")
                             )
...
def project_subset_columns_generate_new_one(spark, my_constant):
    ...
    resultDF = inputDF.select(pyspark.sql.functions.col("LikesTennis"),
                              pyspark.sql.functions.lit(my_constant).alias("NewColumn")
                             )
