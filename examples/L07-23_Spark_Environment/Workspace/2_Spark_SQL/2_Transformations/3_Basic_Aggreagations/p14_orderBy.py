...
# ------------------------------------------
# FUNCTION orderBy_single_column
# ------------------------------------------
def orderBy_single_column(spark):
    ...
    # 3. Operation T1: Transformation orderBy
    resultDF = inputDF.orderBy(inputDF["Name"].asc())
...
# ------------------------------------------
# FUNCTION orderBy_multiple_columns
# ------------------------------------------
def orderBy_multiple_columns(spark):
    # 1. We define the Schema of our DF.
    my_schema = pyspark.sql.types.StructType([pyspark.sql.types.StructField("Name", pyspark.sql.types.StringType(), True),
                                              pyspark.sql.types.StructField("Val1", pyspark.sql.types.IntegerType(), True),
                                              pyspark.sql.types.StructField("Val2", pyspark.sql.types.IntegerType(), True),
    ...
    # 3. Operations GroupBy
    resultDF = inputDF.orderBy([inputDF["Val1"].asc(), inputDF["Name"].desc()])
...
def my_main(spark, option):
    if (option == 1):
        ...
        orderBy_single_column(spark)

    if (option == 2):
        print("\n\n--- OrderBy multiple columns ---\n\n")
        orderBy_multiple_columns(spark)
