...
# ------------------------------------------
# FUNCTION add_column_with_random_value
# ------------------------------------------
def add_column_with_random_value(spark):
    ...
    resultDF = inputDF.withColumn("NewNumber", pyspark.sql.functions.rand())
...
# --------------------------------------------
# FUNCTION add_column_with_conditional_value
# --------------------------------------------
def add_column_with_conditional_value(spark):
    # 1. We define the Schema of our DF.
    my_schema = pyspark.sql.types.StructType([pyspark.sql.types.StructField("Name", pyspark.sql.types.StringType(), True),
                                              pyspark.sql.types.StructField("Number", pyspark.sql.types.IntegerType(), True)
                                             ]
    ...
    # 3. Operation T1: Transformation to create a new column
    resultDF = inputDF.withColumn("NewNumber", pyspark.sql.functions.when(pyspark.sql.functions.rand() > 0.5, 1).otherwise(None))
...
def my_main(spark, option):
    if (option == 1):
        print("\n\n--- Add column with random value ---\n\n")
        add_column_with_random_value(spark)

    if (option == 2):
        print("\n\n--- Add column with conditional value ---\n\n")
        add_column_with_conditional_value(spark)
