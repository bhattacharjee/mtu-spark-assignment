...
object MyProgram {
    ...
    def myMain(sc: SparkContext, myDatasetDir: String): Unit = {
        ...
        // 8. Operation P1: Persist
        solutionRDD.persist()
      
        // 9. Operation A1: Action 'collect'
        val resVAL1: Array[(Int, String)] = solutionRDD.collect()
        for (item <- resVAL1){
          println(item)
        }
      
        // 10. Operation A2: Action 'count'
        val resVAL2: Long = solutionRDD.count()
        println(resVAL2)
    }
