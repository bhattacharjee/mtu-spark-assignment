...
def process_line(line):
    ...
    # 2. We create the output variable
    word_list = []

    # 3. We set the line to be split by " "
    line = line.replace("\n", "")
    line = line.strip()
    line = line.rstrip()
    line = line.replace("\t", " ")

    # 4. We get rid of chars not being either a letter or a " "
    index = len(line) - 1

    # 5. We traverse all characters
    while (index >= 0):
        # 5.1. We get the ord of the character at position index
        char_val = ord(line[index])

        # 5.2. If (1) char_val is not " " and (2) char_val is not an Upper Case letter and (3) char_val is not a Lower Case letter
        if ( ( char_val != 32) and ((char_val < 65) or (char_val > 90)) and ((char_val < 97) or (char_val > 122)) ):
            # 5.2.1. We remove the index from the sentence
            line = line[:index] + line[(index+1):]
        # 5.3. If the character was an upper case letter
        elif ((char_val >= 65) and (char_val <= 90)):
            # 5.3.1. We add it as lower case
            line = line[:index] + chr(char_val + 32) + line[(index + 1):]

        # 5.4. We continue with the next index
        index = index - 1

    # 6. We get the list of words
    word_list = line.split(" ")

    index = len(word_list) - 1

    # 6.1. We traverse the words
    while (index >= 0):
        # 6.1.1. If it is empty, we remove it
        if (word_list[index] == ''):
            del word_list[index]

        # 6.1.2. We continue with the next word
        index = index - 1

    # 7. We get the length of word_list
    res = len(word_list)
...
def my_model(ssc, monitoring_dir, result_dir, time_step_interval, window_duration, sliding_duration):
    ...
    wordsPerLineDStream = inputDStream.map(process_line)

    # 3. Operation T2: reduceByWindow
    solutionDStream = wordsPerLineDStream.reduceByWindow(lambda x, y: x + y,
                                                         lambda x, y: x - y,
                                                         window_duration * time_step_interval,
...
def my_main(sc,
    ...
    ssc.start()
    ssc.awaitTerminationOrTimeout(sliding_duration * time_step_interval)
