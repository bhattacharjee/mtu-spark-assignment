...
def my_model(spark, monitoring_dir, time_step_interval):
    ...
    solutionSDF = word_listSDF.withColumn("word", pyspark.sql.functions.explode(pyspark.sql.functions.col("wordList"))) \
                           .drop("wordList")

    # 6. Operation T3: ERROR - We cannot aggregate without watermark
    #aggSDF = wordsSDF.groupBy(pyspark.sql.functions.col("word")) \
    #                 .count()

    # 7. Operation T6: ERROR - We cannot sort in Append Mode
